 ...
 * 
 * @param {browser.declarativeNetRequest.Rule} newRuleJson The JSON object representing the new rule to add
        ...
        if (!enabled || newRuleJson.timingMode === "afterLoad") {
            ...
            utils.getDynamicRules()
                .then((dynamicRules) => {
                    ...
                    if (numDynamicRules < browser.declarativeNetRequest.MAX_NUMBER_OF_DYNAMIC_AND_SESSION_RULES) {
                        browser.declarativeNetRequest.updateDynamicRules({ addRules: [newRuleCopy] })
                            .then(() => {
            ...
            ruleList.forEach((rule) => {
                if (rule.group !== "GlobalWhitelist" && rule.action.redirect.transform.queryTransform.removeParams[0] === parameter) {
                    if (checkbox.checked) {
                        ...
                        browser.declarativeNetRequest.updateDynamicRules({ removeRuleIds: [rule.id] })
                            .then(() => {
                    ...
                    browser.storage.local.set({ rules: ruleList })
                        .then(() => {
            ...
            .then((ruleList) => {
                ...
                browser.storage.local.set({ rules: ruleList })
                    .then(() => {
                        ...
                        browser.declarativeNetRequest.updateDynamicRules({ removeRuleIds: [parsedRuleId] })
                            .then(() => {
 ...
 * 
 * @param {browser.declarativeNetRequest.Rule} rule The rule whose delete button was clicked.
 */
